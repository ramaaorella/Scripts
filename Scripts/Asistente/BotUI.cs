...
public class BotUI : MonoBehaviour
{
    ...
    public Button Send;                                           // reference to bot chat bubble prefab
    ...
    public void UpdateDisplay(string sender, string message, string messageType)
    {
        ...
        // Set chat bubble position
        StartCoroutine(SetChatBubblePosition(chatBubbleChild.transform.parent.GetComponent<RectTransform>(), sender, messageType));
    }
    ...
    /// <param name="chatBubblePos">RectTransform of chat bubble</param>
    /// <param name="sender">Sender who sent the message</param>
    private IEnumerator SetChatBubblePosition(RectTransform chatBubblePos, string sender, string type)
    {
        // Wait for end of frame before calculating UI transform
        yield return new WaitForEndOfFrame();
    ...
    /// Coroutine to update chat bubble positions based on their size.
    /// </summary>
    public IEnumerator RefreshChatBubblePosition(bool button)
    {
        // Wait for end of frame before calculating UI transform
        yield return new WaitForEndOfFrame();
        ...
        foreach (RectTransform chatBubbleRect in contentDisplayObject.GetComponent<RectTransform>())
        {
            ...
            if (!button)
                ...
                chatBubbleRect.anchoredPosition3D = new Vector3(chatBubbleRect.anchoredPosition3D.x, -localAllMessagesHeight - 25, 0);
        }
    ...
    private void AddChatComponent(GameObject chatBubbleObject, string message, string messageType)
    {
        switch (messageType)
        {
            ...
            case "attachment":
                break;
            case "buttons":
                Button button = Instantiate(Send);
                button.transform.SetParent(chatBubbleObject.transform, false);
                /*Text txt = chatBubbleObject.AddComponent<Text>();
                txt.transform.parent=button.transform);*/
                int i = message.Length - message.IndexOf("/");
                StartCoroutine(networkManager.SetButton(message.Substring(0, message.IndexOf("/")), message.Substring(message.IndexOf("/"), i), button));
                // add font as it is none at times when creating text component from script
                break;
            case "elements":
                break;
